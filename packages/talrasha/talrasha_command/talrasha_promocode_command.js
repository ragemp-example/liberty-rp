module.exports = {
    "promocode_create": {
        ...
        handler: (player, args) => {
            ...
            DB.Handle.query("SELECT name FROM talrasha_character WHERE id=?", [args[0]], (e, resultp) => {
                ...
                var namePromotion = resultp[0]['name'];
                    DB.Handle.query("SELECT * FROM talrasha_promocode WHERE character_id=? or code=?", [args[0],args[1]], (e, resultcode) => {
                        if (resultcode.length > 0) return terminal.info(`Ошибка создания промо-кода - промокод уже создан!`); 
                            DB.Handle.query("INSERT INTO talrasha_promocode (character_id,code,add_admin,reward_config) VALUES (?,?,?,?)",
    ...
    "promocode_edit": {
        ...
        handler: (player, args) => {
            ...
            DB.Handle.query(`UPDATE talrasha_promocode SET reward_config=? WHERE code=?`,[JSON.stringify(code_settings),args[0]], (e,result) => {
                    if(result.length === 0){
