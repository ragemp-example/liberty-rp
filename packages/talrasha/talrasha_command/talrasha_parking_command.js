module.exports = {
    "parking_create": {
        ...
        handler: (player, args) => {
            ...
            DB.Handle.query("INSERT INTO talrasha_parking (name,cords) VALUES (?,?)",
                [args[0], JSON.stringify(player.position)], (e, result) => {
                    DB.Handle.query("SELECT * FROM talrasha_parking WHERE id=?", result.insertId, (e, result) => {
    ...
    "parking_create_slot": {
        ...
        handler: (player, args) => {
            ...
            DB.Handle.query("INSERT INTO talrasha_parking_slot (pid,cords) VALUES (?,?)",
                [args[0], JSON.stringify(player.position)], (e, result) => {
                    DB.Handle.query("SELECT * FROM talrasha_parking_slot WHERE id=?", result.insertId, (e, result) => {
    ...
    "parking_delete_slot": {
        ...
        syntax: "[idSlot]:n",
        handler: (player, args) => {
            DB.Handle.query("SELECT * FROM talrasha_parking_slot WHERE id=?", [args[0]], (e, result) => {
                if (!e) {
