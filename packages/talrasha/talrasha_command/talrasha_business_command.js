module.exports = {
	...
	"set_biz_extraPoint": {
        description: "Изменить дополнительную точку бизнеса.",
        minLevel: 5,
        syntax: "[ид_бизнеса]:n",
        handler: (player, args) => {
            var biz = mp.bizes.getBySqlId(args[0]);
            if (!biz) return terminal.error(`Бизнес с ID: ${args[0]} не найден!`, player);
            const coords = {pos: player.position, rotate: player.heading};
            biz.setExtraPoint(JSON.stringify(coords));
            terminal.info(`${player.name} изменил дополнительную точку у бизнеса с ID: ${args[0]}`);
        }
    },
    "set_biz_position": {
