  module.exports = {
    "getpos": {
        ...
        handler: (player, args) => {
            ...
            if(player.vehicle) {
				...
				debug(`{"x":${player.vehicle.position.x.toFixed(2)},"y":${player.vehicle.position.y.toFixed(2)},"z":${player.vehicle.position.z.toFixed(2)},"rot":${player.vehicle.heading.toFixed(2)},"dim":0}`)
            } else {
    ...
    "clothes": {
        ...
        handler: (player, args) => {
            if (args[0] == 11) {
				player.setVariable("clothes_top_variation", args[1]);
				player.setVariable("clothes_top_texture", args[2]);
				return
			}
        ...
        handler: (player, args) => {
          DB.Handle.query("SELECT * FROM talrasha_spawn_pos", (e, result) => {
            if (e) {
              ...
              DB.Handle.query("INSERT INTO talrasha_spawn_pos (name, x, y, z, rot) VALUES (?, ?, ?, ?, ?)", [args[0], pos.x.toFixed(3), pos.y.toFixed(3), pos.z.toFixed(3), player.heading.toFixed(3)], (e, sresult) => {
                    if (e) {
                      callback("Ошибка выполнения запроса в БД!");
                      return terminal.error(e);
                    }
    ...
    "delete_spawn": {
        ...
        syntax: "[id]:n",
        handler: (player, args) => {
          DB.Handle.query("SELECT * FROM talrasha_spawn_pos WHERE id=?", [args[0]], (e, result) => {
              if (e) {
                callback("Ошибка выполнения запроса в БД!");
                return terminal.error(e);
              }
              if (result.length < 1) return terminal.log("Спавн #" + args[0] + " не существует!", player);
              DB.Handle.query(`DELETE FROM talrasha_spawn_pos WHERE id=?`, args[0], (e, result) => {
                  if (e) {
                      callback("Ошибка выполнения запроса в БД!");
                      return terminal.error(e);
                    }
    ...
    "select_flight": {
        ...
        handler: (player, args) => {
          ...
          DB.Handle.query("SELECT * FROM talrasha_bus_route WHERE id=?", [args[0]], (e, result) => {
              if (e) {
                callback("Ошибка выполнения запроса в БД!");
                return terminal.error(e);
              }
              ...
              player.marsh = args[0];
              let arr = [];
              DB.Handle.query("SELECT * FROM talrasha_bus_place WHERE bus=?", [args[0]], (e, sresult) => {
    ...
    "create_point": {
        ...
        handler: (player, args) =>  {
          ...
          DB.Handle.query("SELECT * FROM talrasha_bus_route WHERE id=?", [player.marsh], (e, result) => {
              if (e) {
                callback("Ошибка выполнения запроса в БД!");
                return terminal.error(e);
              }
              ...
              if (args[0] === 0 || args[0] === 1) {
                DB.Handle.query("INSERT INTO talrasha_bus_place (bus, x, y, z, type) VALUES (?, ?, ?, ?, ?)", [player.marsh, player.position.x, player.position.y, player.position.z, args[0]], (error, sresult) => {
                    if (error) {
                        callback("Ошибка выполнения запроса в БД!");
    ...
    "delete_point": {
        ...
        handler: (player, args) =>  {
          if (!player.marsh) return terminal.log("Вы не редактируете маршрут!", player);
          DB.Handle.query("SELECT * FROM talrasha_bus_route WHERE id=?", [player.marsh], (e, sresult) => {
              ...
              DB.Handle.query("SELECT * FROM talrasha_bus_place WHERE id=?", [args[0]], (e, result) => {
                if (e) { callback("Ошибка выполнения запроса в БД!"); return terminal.error(e); }
                ...
                DB.Handle.query(`DELETE FROM talrasha_bus_place WHERE id=?`, args[0], (e, results) => {
                    if (e) {
                        callback("Ошибка выполнения запроса в БД!");
                        return terminal.error(e);
                      }
        ...
        handler: (player, args) => {
          DB.Handle.query("SELECT * FROM talrasha_bus_route", (e, result) => {
            if (e) {
              ...
              let id = result.length + 1;
              DB.Handle.query("INSERT INTO talrasha_bus_route (name) VALUES (?)", [args[0]], (e, sresult) => {
                    if (e) {
                      callback("Ошибка выполнения запроса в БД!");
                      return terminal.error(e);
                    }
    ...
    "delete_flight": {
        ...
        handler: (player, args) => {
          DB.Handle.query("SELECT * FROM talrasha_bus_route WHERE id=?", [args[0]], (e, result) => {
              ...
              DB.Handle.query(`DELETE FROM talrasha_bus_route WHERE id=?`, args[0], (e, result) => {
                  if (e) {
                      callback("Ошибка выполнения запроса в БД!");
                      return terminal.error(e);
                    }
        ...
        handler: (player, args) =>  {
          ...
          DB.Handle.query("SELECT * FROM talrasha_bus_route WHERE id=?", [player.marsh], (e, sresult) => {
              ...
              DB.Handle.query("SELECT * FROM talrasha_bus_place WHERE id=?", [args[0]], (e, result) => {
                ...
                DB.Handle.query("UPDATE talrasha_bus_place SET x=?,y=?,z=? WHERE id=?", [player.position.x, player.position.y, player.position.z, args[0]]);
              });
          });
        }
    },
          ...
          DB.Handle.query("SELECT * FROM talrasha_bus_route WHERE id=?", [player.marsh], (e, sresult) => {
              ...
              if (args[1] === 0 || args[1] === 1) {
                DB.Handle.query("SELECT * FROM talrasha_bus_place WHERE id=?", [args[0]], (e, result) => {
                  ...
                  DB.Handle.query("UPDATE talrasha_bus_place SET type=? WHERE id=?", [args[1], args[0]]);
                });
              } else {
                terminal.log("0 - Промежуточный чекпоинт | 1 - Остановка");
              }
          ...
          DB.Handle.query("SELECT * FROM talrasha_bus_route", (e, result) => {
              ...
              for (let i = 0; i < result.length; i++) {
                ...
                if (result[i].id === args[0]) {
                  DB.Handle.query("UPDATE talrasha_bus_route SET name=? WHERE id=?", [args[1], args[0]], (e, result) => {
                      if (e) {
                          callback("Ошибка выполнения запроса в БД!");
                          return terminal.error(e);
                        }
    ...
    "change_salary_flight": {
        ...
        handler: (player, args) => {
          DB.Handle.query("SELECT * FROM talrasha_bus_route WHERE id=?", [args[0]], (e, result) => {
              ...
              if (result.length < 1) return terminal.log("Маршрут #" + args[0] + " не существует!", player);
              DB.Handle.query("UPDATE talrasha_bus_route SET money=? WHERE id=?", [args[1], args[0]], (e, result) => {
                  if (e) {
                      callback("Ошибка выполнения запроса в БД!");
                      return terminal.error(e);
                    }
    ...
    "calltaxi": {
        ...
        syntax: "",
        handler: (player, args) => {
			let taxiOpen = require("../talrasha_module/talrasha_job/talrasha_taxi/talrasha_taxi.js");
