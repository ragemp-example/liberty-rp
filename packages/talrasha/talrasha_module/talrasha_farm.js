...
function loadFarmsFromDB() {
    mp.farms = [];
    DB.Handle.query("SELECT * FROM talrasha_farm", (e, result) => {
		...
		console.log(`\x1b[32m[DONE]\x1b[0m "Farms" package has been loaded: \x1b[33m${i}\x1b[0m.`);
    });
    initFarmsUtils();
}
...
function loadFarmFieldsFromDB() {
    mp.farmFields = [];
    DB.Handle.query("SELECT * FROM talrasha_farm_field", (e, result) => {
        for (var i = 0; i < result.length; i++) {
		...
		console.log(`\x1b[32m[DONE]\x1b[0m "Farm fields" package has been loaded: \x1b[33m${i}\x1b[0m.`);
    });
    initFarmFieldsUtils();
}
...
function initFarmUtils(farm) {
    farm.setBalance = (balance) => {
        ...
        DB.Handle.query("UPDATE talrasha_farm SET balance=? WHERE id=?", [farm.balance, farm.sqlId]);
    }
    farm.setTaxBalance = (newTaxBalance) => {
        farm.taxBalance = Math.clamp(newTaxBalance, 0, 1200);
        DB.Handle.query("UPDATE talrasha_farm SET taxBalance=? WHERE id=?", [farm.taxBalance, farm.sqlId]);
    }
    farm.sellToGos = () => {
        ...
        if (oldOwner) {
            ...
            oldOwner.utils.info(`Начислено: ${fullSum}$`);
        } else {
            DB.Handle.query(`UPDATE talrasha_character SET bank=bank+? WHERE id=?`, [fullSum, farm.owner]);
        }
        ...
        var query = "UPDATE talrasha_farm SET grains=?,pumpkins=?,cabbages=?,weeds=?,pumpkinPrice=?,cabbagePrice=?,weedPrice=?,grainPrice=?,owner=?,balance=?,taxBalance=?,pay=? WHERE id=?";
    ...
    farm.setGrains = (grains) => {
        ...
        DB.Handle.query("UPDATE talrasha_farm SET grains=? WHERE id=?", [farm.grains, farm.sqlId]);
    ...
    farm.setPumpkins = (count) => {
        ...
        DB.Handle.query("UPDATE talrasha_farm SET pumpkins=? WHERE id=?", [farm.pumpkins, farm.sqlId]);
    ...
    farm.setCabbages = (count) => {
        ...
        DB.Handle.query("UPDATE talrasha_farm SET cabbages=? WHERE id=?", [farm.cabbages, farm.sqlId]);
        farm.cabbageLabel.text = `~y~Капуста: ~w~${farm.cabbages} из 2000 ед.\n~g~Цена: ~w~${farm.cabbagePrice}$`;
    }
    farm.setWeeds = (count) => {
        farm.weeds = Math.clamp(count, 0, 2000);
        DB.Handle.query("UPDATE talrasha_farm SET weeds=? WHERE id=?", [farm.weeds, farm.sqlId]);
...
function initFarmFieldUtils(field) {
    field.setCount = (count) => {
        ...
        if (field.count % 20 == 0) DB.Handle.query("UPDATE talrasha_farm_field SET count=? WHERE id=?", [field.count, field.sqlId]);
    ...
    field.setCropType = (cropType) => {
        ...
        DB.Handle.query("UPDATE talrasha_farm_field SET type=? WHERE id=?", [field.cropType, field.sqlId]);
    };
    field.fill = (type) => {
        ...
        field.count = 600;
        DB.Handle.query("UPDATE talrasha_farm_field SET type=?,count=? WHERE id=?", [field.cropType, field.count, field.sqlId]);
        field.label.text = `~y~Поле ~w~№${field.sqlId}\n ~y~Урожай: ~w~${cropNames[field.cropType]}`;
