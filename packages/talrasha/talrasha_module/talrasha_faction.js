module.exports = {
    Init: () => {
        ...
        mp.factionRanks = {};
        initFactionsUtils();
        DB.Handle.query("SELECT * FROM talrasha_faction", (e, result) => {
			...
			console.log(`\x1b[32m[DONE]\x1b[0m "Faction" package has been loaded: \x1b[33m${i}\x1b[0m.`);


            mp.factions.forEach((faction) => {
                mp.factionRanks[faction.sqlId] = [];
            });
            DB.Handle.query("SELECT * FROM talrasha_faction_rank", (e, result) => {
                for (var i = 0; i < result.length; i++) {
				...
				console.log(`\x1b[32m[DONE]\x1b[0m "Faction ranks" package has been loaded: \x1b[33m${i}\x1b[0m.`);
            });
...
function initFactionsUtils() {
    ...
    mp.factions.setRankName = (factionId, rank, name) => {
        ...
        DB.Handle.query("UPDATE talrasha_faction_rank SET name=? WHERE factionId=? AND rank=?", [name, factionId, rank]);
    };
    mp.factions.setRankPay = (factionId, rank, pay) => {
        ...
        DB.Handle.query("UPDATE talrasha_faction_rank SET pay=? WHERE factionId=? AND rank=?", [pay, factionId, rank]);
    };
	mp.factions.isBandFaction = (factionId) => {
        return factionId  >= 9 && factionId <= 21;
    };
	...
	mp.factions.isGoverFaction = (factionId) => {
        return factionId == 1;
    };
...
function initFactionUtils(faction) {
    faction.setName = (name) => {
        ...
        DB.Handle.query("UPDATE talrasha_faction SET name=? WHERE id=?", [faction.name, faction.sqlId]);
    };
    faction.setLeader = (sqlId, name) => {
        ...
        if (faction.leader > 0) {
            ...
            else DB.Handle.query("UPDATE talrasha_character SET `faction`=?,`rank`=? WHERE `id`=?", [0, 0, faction.leader]);
        ...
        else DB.Handle.query("UPDATE talrasha_character SET `faction`=?,`rank`=? WHERE `id`=?", [faction.sqlId, mp.factionRanks[faction.sqlId].length - 1, sqlId]);

        faction.leader = sqlId;
        faction.leaderName = name;
        DB.Handle.query("UPDATE talrasha_faction SET leader=?,leaderName=? WHERE id=?", [sqlId, name, faction.sqlId]);
    };
    faction.setProducts = (products) => {
        faction.products = Math.clamp(products, 0, faction.maxProducts);
        DB.Handle.query("UPDATE talrasha_faction SET products=? WHERE id=?", [faction.products, faction.sqlId]);
    };
    faction.setMaxProducts = (maxProducts) => {
        if (maxProducts < 1) maxProducts = 1;
        faction.maxProducts = maxProducts;
        DB.Handle.query("UPDATE talrasha_faction SET maxProducts=? WHERE id=?", [faction.maxProducts, faction.sqlId]);
    };
    faction.setBlipColor = (blipColor) => {
        if (blipColor < 1) blipColor = 1;
        faction.blip.color = blipColor;
        DB.Handle.query("UPDATE talrasha_faction SET blipColor=? WHERE id=?", [blipColor, faction.sqlId]);
    };
	faction.setBalance = (balance) => {
        if (balance < 0) balance = 0;
        faction.balance = balance;
        DB.Handle.query("UPDATE talrasha_faction SET balance=? WHERE id=?", [balance, faction.sqlId]);
    };
    faction.setPosition = (pos, heading) => {
        ...
        DB.Handle.query("UPDATE talrasha_faction SET x=?,y=?,z=?,h=? WHERE id=?", [pos.x, pos.y, pos.z, faction.h, faction.sqlId]);
    };
    faction.setWarehousePosition = (pos) => {
        ...
        DB.Handle.query("UPDATE talrasha_faction SET wX=?,wY=?,wZ=? WHERE id=?", [pos.x, pos.y, pos.z, faction.sqlId]);
    };
    faction.setStoragePosition = (pos) => {
        ...
        DB.Handle.query("UPDATE talrasha_faction SET sX=?,sY=?,sZ=? WHERE id=?", [pos.x, pos.y, pos.z, faction.sqlId]);
    };
}
...
var menuNames = ['gover_storage', 'police_storage', 'police_storage_2', 'fib_storage', 'hospital_storage', 'army_storage',
    'army_storage_2', 'news_storage', 'band_storage', 'band_storage', 'band_storage', 'band_storage', 'mafia_storage',
    'mafia_storage', 'mafia_storage', 'mafia_storage', 'band_storage', 'band_storage', 'mafia_storage',  'mafia_storage', 
	'band_storage', 'band_storage', 'band_storage', 'band_storage', 'band_storage', 'band_storage', 'band_storage', 'band_storage',
	'band_storage', 'band_storage', 'band_storage'
];
var propNames = ['goverStorage', 'policeStorage', 'policeStorage', 'fibStorage', 'hospitalStorage',
    'armyStorage', 'armyStorage', 'newsStorage', 'bandStorage', 'bandStorage', 'bandStorage', 'bandStorage', 'mafiaStorage',
    'mafiaStorage', 'mafiaStorage', 'mafiaStorage', 'bandStorage', 'bandStorage', 'mafiaStorage', 'mafiaStorage', 'bandStorage',
	'bandStorage', 'bandStorage', 'bandStorage', 'bandStorage', 'bandStorage', 'bandStorage', 'bandStorage', 'bandStorage', 'bandStorage', 'bandStorage'
];
