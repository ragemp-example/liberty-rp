const barberInfo = require("./talrasha_info");
...
mp.events.add("barbershop::setHair", (player, hair, color, highlightColor) => {
	...
	DB.Handle.query("UPDATE talrasha_character SET hair=?, hairColor=?, hairHighlightColor=? WHERE id=?", [hair, color, highlightColor, player.sqlId]);
});

mp.events.add("barbershop::setEyeColor", (player, color) => {
	player.eyeColor = color;

	DB.Handle.query("UPDATE talrasha_character SET eyeColor=? WHERE id=?", [color, player.sqlId]);
});
...
mp.events.add("barbershop::setHeadOverlay", (player, overlayId, index, opacity, color, clearOverlayId) => {
	...
	DB.Handle.query(`
		INSERT INTO 
			talrasha_character_overlay(character_id, overlay_id, overlay_index, opacity, first_color, second_color)
		VALUES 
			(?, ?, ?, ?, ?, ?)
	...
	if (typeof(clearOverlayId) === "number") {
		...
		DB.Handle.query("DELETE FROM talrasha_character_overlay WHERE character_id = ? AND overlay_id = ?", [player.sqlId, clearOverlayId]);
	}
});
