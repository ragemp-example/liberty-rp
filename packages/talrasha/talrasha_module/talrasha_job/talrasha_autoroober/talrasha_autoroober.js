...
function CreateRooberPlace(player) {
    try
    {
        ...
        let veh = mp.vehicles.new(mp.joaat(vehname), new mp.Vector3(pos.x, pos.y, pos.z), { heading: pos.rot, numberPlate: "TalRasha", color: [[col, col, col],[col, col, col]] });
        veh.roober = player;
    ...
    try
    {
        if (!player.vehicle) {
            if (shape == rooberColShape) {
                return;
				player.call("prompt.show", ["<span>Е</span> Нажми для взаимодействия"]);
                player.setVariable("keydownevariable2", true);
            }
        } else {
             if (shape == rooberColShape) if (player.vehicle.roober !== undefined) if (player.vehicle.roober === player) PassRooberVehicle(player, 9999);
        }
    }
    catch (err){
        console.log(err);
...
console.log("\x1b[32m[DONE]\x1b[0m Carroober job have been started.");
