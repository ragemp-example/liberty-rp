mp.blips.new(103, new mp.Vector3(538.54, 101.79, 96.54), { name: 'Курьер пиццерии', color: 1, scale: 1, shortRange: true}); // Блип на карте
/*
...
mp.events.add("playerEnterVehicle", function playerEnterVehicleHandler(player, vehicle, seat) {
  // player.notify("Модель: ~r~" + vehicle.model);
	if (vehicle.owner === -4 && player.job === 4 && seat === 0) {
    let skill = player.jobSkills[4 - 1];
