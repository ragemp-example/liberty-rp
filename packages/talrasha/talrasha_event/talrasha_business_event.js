module.exports = {
    ...
    "biz.balance.get": (player) => {
        ...
        player.utils.info(`На кассе ${biz.name} ${biz.balance}$`);
    },
	"clothesShop.open": (player, add, newmenu) => {
        if (add) {
			const dim = player.sqlId + 156;
			player.dimension = dim;
			player.isClothesshop = true;
			player.call("clothesShop.open", [true, newmenu, player.colshape.biz.sqlId]);
		}
		else {
			delete player.isClothesshop;
			player.dimension = 0;
			player.call("clothesShop.open", [false]);
    ...
    "biz.balance.add": (player, value) => {
        ...
        player.utils.info(`Касса ${biz.name} пополнена на ${value}$. Всего: ${biz.balance + value}$.`);
    ...
    "biz.balance.take": (player, value) => {
        ...
        player.utils.info(`С кассы ${biz.name} снято ${value}$. Всего: ${biz.balance - value}$.`);
        biz.setBalance(biz.balance - value);
        player.utils.setMoney(player.money + value);
    },
    "biz.getStats": (player, offset) => {
        ...
        DB.Handle.query("SELECT * FROM talrasha_business_log WHERE bizId=? ORDER BY id DESC LIMIT ?,30", [biz.sqlId, offset], (e, result) => {
    ...
    "biz.products.buy": (player, products) => {
        ...
        player.utils.info(`Склад ${biz.name} пополнен на ${products} ед. товара. Всего: ${biz.products + products}/${biz.maxProducts} ед.`);
    ...
    "biz.products.sell": (player, products) => {
        ...
        player.utils.info(`Со склада ${biz.name} списано ${products} ед. товара. Всего: ${biz.products - products}/${biz.maxProducts} ед.`);
        biz.setProducts(biz.products - products);
        player.utils.setMoney(player.money + sum);
    },
    "biz.products.price": (player, productPrice) => {
        ...
        player.utils.info(`Новая цена товара: ${biz.productPrice}$`);
    },
    "biz.setStatus": (player, status) => {
        ...
        if (status == 0) player.utils.info(`Бизнес ${biz.name} закрыт!`);
        else player.utils.info(`Бизнес ${biz.name} открыт!`);
    ...
    "biz_3.clearItems": (player) => {
        if (!player.colshape || !player.colshape.biz) return player.utils.error(`Вы не у бизнеса!`);
        var biz = player.colshape.biz;
        if (biz.bizType != 3 && biz.bizType != 18) return player.utils.error(`Неверный тип бизнеса!`);
    ...
    "biz_3.buyItem": (player, type, index, texture) => {
        ...
        if (!player.colshape || !player.colshape.biz) return player.utils.error(`Вы не у бизнеса!`);
        var biz = player.colshape.biz;
        if (biz.bizType != 3 && biz.bizType != 13 && biz.bizType != 18 && biz.bizType != 20) return player.utils.error(`Неверный тип бизнеса!`);
        if (!biz.status) return player.utils.error(`Бизнес закрыт!`);

        var clothes = mp.clothes[type][type == "masks" ? 1 : player.sex][index];
        ...
        var types = ["top","legs","feets","hats","glasses","bracelets","ears","masks","ties","watches", "undershirts", "bags"];
        var itemIds = [7,8,9,6,1,12,10,14,2,11,15,13];
