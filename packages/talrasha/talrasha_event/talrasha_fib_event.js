...
module.exports = {
    "fibStorage.takeClothes": (player, index) => {
		...
		mp.logs.storage(player.name, player.sqlId, player.faction, mp.factionRanks[player.faction][player.rank].name, 'Взял', 1, 'Форма FIB')
    ...
    "fibStorage.takeArmour": (player) => {
        ...
        player.inventory.add(3, params, {}, (e) => {
			...
			mp.logs.storage(player.name, player.sqlId, player.faction, mp.factionRanks[player.faction][player.rank].name, 'Взял', 1, 'Бронежилет FIB')
    ...
    "fibStorage.takeGun": (player, index) => {
        ...
        var itemIds = [17, 18, 19, 20, 21, 22, 23, 66];
        var weaponIds = ["weapon_nightstick", "weapon_flashlight", "weapon_stungun", "weapon_combatpistol", "weapon_pumpshotgun", "weapon_carbinerifle", "weapon_sniperrifle","weapon_heavysniper_mk2"];
