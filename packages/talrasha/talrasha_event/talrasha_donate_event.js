module.exports = {
    "donateSystem.convertMoney": (player, donate) => {
        if(player.account.donate < donate) return player.utils.error(`У вас недостаточно LRP-коинов!`);
        if(donate <= 0) return player.utils.error(`Вы не можете конвертировать нуль LRP-коинов!`);
    ...
    "donateSystem.buyPackage": (player, package) => {
        ...
        if(pack.price > player.account.donate) return player.utils.error(`У вас недостаточно LRP-коинов!`);
        if(pack.price <= 0) return player.utils.error(`Вы не можете конвертировать нуль LRP-коинов!`);
        if(pack.type == 1) {
    ...
    "donateSystem.buyExtension": (player, extension) => {
        ...
        if(pack.sqlId < 7) {
            if(pack.sqlId == 6) {
                ...
                DB.Handle.query("UPDATE talrasha_character SET donateBizes = ? WHERE id = ?", [player.donateBizes, player.sqlId]);
                ...
                DB.Handle.query("UPDATE talrasha_character SET donateHouse = ? WHERE id = ?", [player.donateHouse, player.sqlId]);
            ...
            player.utils.setDonate(player.account.donate - pack.price);
            player.donateCars += 1;
            DB.Handle.query("UPDATE talrasha_character SET donateCars = ? WHERE id = ?", [player.donateCars, player.sqlId]);
            player.utils.success(`Вы успешно приобрели дополнительные слоты для машин. Приятной игры!`);
        }
    },

    "donateSystem.updateDonate": (player) => {
        DB.Handle.query("SELECT * FROM talrasha_account WHERE id = ?", [player.account.id], (e, result) => {
            if (result.length > 0) {
    ...
    "donateSystem.buyVip": (player, package) => {
        ...
        if(pack.price > player.account.donate) return player.utils.error(`У вас недостаточно LRP-коинов!`);
        if(pack.price <= 0) return player.utils.error(`Вы не можете оплатить нуль LRP-коинов!`);
        player.utils.setDonate(player.account.donate - pack.price);
        player.vipDate = pack.time;
        player.utils.success(`Вы успешно приобрели вип-пакет на ${getTime /= 24} дня(ей). Поздравляем!`);
        DB.Handle.query("UPDATE talrasha_character SET vipDate = ? WHERE id = ?", [pack.time, player.sqlId]);
    }
}