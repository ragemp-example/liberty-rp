module.exports = {
    ...
    "vehicle.engine.on": (player) => {
		...
		if (vehicle.owner > 2000 && player.admin <= 0) {
    ...
    "car.fix.accept": (player, sqlId) => {
        if (player.money < 3000) return player.utils.error(`Недостаточно средств!`);
    ...
    "car.buycarplayer.agree": (player) => {
        ...
        player.inventory.add(54, params, null, (e) => {
            ...
            player.cars.push({ name: params.model, sqlId: veh.id, price: veh.price });
