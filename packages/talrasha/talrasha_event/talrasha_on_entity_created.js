        ...
        if (entity.type == "vehicle") {
            ...
            entity.utils = {
                addMileage: (add) => {
                    ...
                    if (entity.sqlId)
                        DB.Handle.query(`UPDATE talrasha_vehicle SET mileage=? WHERE id=?`, [entity.vehPropData.mileage, entity.sqlId]);
                },
                setEngineBroken: (enable) => {
                    ...
                    if (entity.sqlId) {
                        enable = (enable) ? 1 : 0;
                        DB.Handle.query(`UPDATE talrasha_vehicle SET engineBroken=? WHERE id=?`, [enable, entity.sqlId]);
                    }
                },
                setOilBroken: (enable) => {
                    ...
                    if (entity.sqlId) {
                        enable = (enable) ? 1 : 0;
                        DB.Handle.query(`UPDATE talrasha_vehicle SET oilBroken=? WHERE id=?`, [enable, entity.sqlId]);
                    }
                },
                setAccumulatorBroken: (enable) => {
                    ...
                    if (entity.sqlId) {
                        enable = (enable) ? 1 : 0;
                        DB.Handle.query(`UPDATE talrasha_vehicle SET accumulatorBroken=? WHERE id=?`, [enable, entity.sqlId]);
                    }
                },
                setFuel: (fuel) => {
                    //debug(`setFuel: ${fuel}`)
                    ...
                    if (entity.sqlId)
                        DB.Handle.query(`UPDATE talrasha_vehicle SET fuel=? WHERE id=?`, [fuel, entity.sqlId]);
                ...
                setSpawnPos: (pos) => {
                    entity.spawnPos = pos;
                    if (entity.sqlId) DB.Handle.query(`UPDATE talrasha_vehicle SET x=?,y=?,z=?,h=?,dimension=? WHERE id=?`,
                        [pos.x, pos.y, pos.z, pos.h, entity.dimension, entity.sqlId]);
                },
                setLicense: (license) => {
                    var types = [1, 2, 3, 4, 11, 12];
                    if (license && types.indexOf(license) == -1) return;
                    entity.license = parseInt(license);
                    if (entity.sqlId) DB.Handle.query(`UPDATE talrasha_vehicle SET license=? WHERE id=?`,
                        [entity.license, entity.sqlId]);
                },
                setOwner: (owner) => {
                    entity.owner = owner;
                    DB.Handle.query("UPDATE talrasha_vehicle SET owner=? WHERE id=?", [owner, entity.sqlId]);
                },
                setSpawnTimer: (ms) => {
