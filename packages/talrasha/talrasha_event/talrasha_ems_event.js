...
module.exports = {
    "hospitalStorage.takeClothes": (player, index) => {
		...
		mp.logs.storage(player.name, player.sqlId, player.faction, mp.factionRanks[player.faction][player.rank].name, 'Взял', 1, 'Форма Hospital')
    ...
    "hospital.acceptCall": (player, data) => {
		let res = JSON.parse(data)
        if (!mp.factions.isHospitalFaction(player.faction)) return player.utils.error(`Вы не сотрудник больницы!`);
        var rec = mp.players.at(parseInt(res.id));
        if (!rec) return player.utils.error(`Гражданин не найден!`);
