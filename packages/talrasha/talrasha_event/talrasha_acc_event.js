...
module.exports = {
    "sendEmailCode": (player, email, login) => {
        ...
        DB.Handle.query("SELECT login,email FROM talrasha_account WHERE login=? OR (email=? AND confirmEmail=?)", [login, email, 1], (e, result) => {
    ...
    "regAccount": (player, data) => {
        ...
        DB.Handle.query(`SELECT login,email,socialClub FROM talrasha_account WHERE login=? OR socialClub=? OR (email=? AND confirmEmail=?)`, [data.login, player.socialClub, data.email, 1], (e, result) => {
            ...
            if (result.length > 0) {
                ...
                var rewardProm = 0;
                if(data.promocode){
                    DB.Handle.query(`SELECT * FROM talrasha_promocode WHERE code=?`, [data.promocode], (e, result) => {
                        if (result.length > 0) {
                ...
                DB.Handle.query(`INSERT INTO talrasha_account (login,socialClub,password,email,regIp,lastIp,lastDate,confirmEmail,promocode) VALUES (?,?,?,?,?,?,NOW(),?,?)`,
                    values, (e, result) => {
                        if (e) return console.log(e);
                        ...
                        if(promoId!==0){
                            ...
                            DB.Handle.query(`INSERT INTO talrasha_promocode_data (idcode,regId,date,rewarded) VALUES (?,?,NOW(),?)`,promoVals, () => {});
                            DB.Handle.query(`UPDATE talrasha_promocode SET reward_sum=reward_sum+? WHERE id=?`,[rewardProm,promoId], () => {});
                        }
    ...
    "recoveryAccount": (player, loginOrEmail) => {
        ...
        DB.Handle.query("SELECT email FROM talrasha_account WHERE (login=? OR email=?) AND confirmEmail=?", [loginOrEmail, loginOrEmail, 1], (e, result) => {
            if (e) return console.log(e);
            if (result.length == 0) {
                player.call(`lightRecoveryLoginOrEmail`);
    ...
    "recoveryAccountNewPassword": (player, password) => {
        ...
        DB.Handle.query("SELECT id FROM talrasha_account WHERE (login=? OR email=?) AND confirmEmail=?", [player.loginOrEmail, player.loginOrEmail, 1], (e, result) => {
            delete player.loginOrEmail;
            if (e) return console.log(e);
            if (result.length == 0) return player.utils.error(`Учетная запись не найдена!`);

            DB.Handle.query("UPDATE talrasha_account SET password=? WHERE id=?", [md5(password), result[0].id], (e) => {
                if (e) return console.log(e);
    ...
    "authAccount": (player, loginOrEmail, password) => {
        ...
        DB.Handle.query("SELECT * FROM talrasha_ban_ip WHERE ip=?", [player.ip], (e, result) => {
            if (result.length > 0) {
        ...
        DB.Handle.query(`SELECT * FROM talrasha_account WHERE (login=? OR (email=? AND confirmEmail=?)) AND password=?`,
            [loginOrEmail, loginOrEmail, 1, md5(password)], (e, result) => {
                ...
                if(result[0].promocode){
                    ...
                    DB.Handle.query(`SELECT * FROM talrasha_promocode WHERE code=?`,[result[0].promocode], (e, resultCode) => {
				...
				DB.Handle.query(`UPDATE talrasha_account SET lastDate=NOW() WHERE id=?`,[player.account.id], (e,result) => {});
            });
    },
