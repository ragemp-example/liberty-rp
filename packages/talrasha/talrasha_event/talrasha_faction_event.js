module.exports = {
    ...
    "factions.uninvite": (player, recId) => {
		...
		mp.logs.factions(rec.sqlId, player.name, `Уволил игрока ${rec.name} из организации.`) 
    ...
    "factions.offer.agree": (player) => {
		...
		mp.logs.factions(rec.sqlId, player.name, `Принял игрока ${rec.name} в организацию.`)
    ...
    "warehouse.push": (player) => {
        ...
        var model = 'prop_box_ammo04a';
		if (player.colshape.warehouse.faction == 5) model = 'ex_office_swag_pills4';
        var weight = 500;
        if (model != player.getVariable("attachedObject")) return player.utils.error(`Неверный тип товара!`);
        player.setVariable("attachedObject", null);

        var faction = mp.factions.getBySqlId(player.colshape.warehouse.faction);
        if (faction.products + weight > faction.maxProducts) player.utils.warning(`Склад заполнен!`);

        faction.setProducts(faction.products + weight);
