...
const {initTeamPlayers} = require("../talrasha_module/talrasha_ipad/talrasha_ipad");
...
module.exports = {
    ...
    "setSpawn": (player, spawnPoint) => {
        ...
        DB.Handle.query(`UPDATE talrasha_character SET spawn = ? WHERE id = ?`,
    ...
    "setHouseId": (player, houseId) => {
        ...
        DB.Handle.query(`UPDATE talrasha_character SET houseId = ? WHERE id = ?`,
    ...
    "regCharacter": (player, rawData) => {
        ...
        DB.Handle.query("SELECT null FROM talrasha_character WHERE accountId=?", [player.account.id], (e, result) => {
            ...
            /*if (result.length === 0) {
                ...
                DB.Handle.query(`UPDATE talrasha_account SET promocode='' WHERE id=?`, [player.account.id], (e, result) => {});
            } else {
                var PromoId = 0;
            }*/

            DB.Handle.query(`SELECT null FROM talrasha_character WHERE name=?`, [data.characterName], (e, result) => {
                if (e) {
				...
				let phonenumber = mp.Phone._generatePhonenumber()
				phonenumber.then((resolve) => {   
					var query = `INSERT INTO talrasha_character (name,regIp,sex,lastIp,mother,father,shapeMix,skinMix,
					...
					var values = [data.characterName, player.ip, player.sex, player.ip, data.mother,
						...
						player.account.id, pos.x, pos.y, pos.z, pos.h, startMoney, resolve
					];
					...
					DB.Handle.query(query, values, (e, result) => {
						...
						const headOverlayQuery = "INSERT INTO talrasha_character_overlay(character_id, overlay_id, overlay_index, opacity, first_color, second_color) VALUES ?";
						...
						for (var i = 1; i < bodyItems.length; i++) {
							...
							DB.Handle.query("INSERT INTO talrasha_player_inventory (playerId,itemId,`index`,params) VALUES (?,?,?,?)",
						...
						[result.insertId, item.itemId, item.itemId - 1, JSON.stringify(item.params)], (e, result2) => {
							...
							DB.Handle.query("INSERT INTO talrasha_player_inventory (playerId,itemId,`index`,params,parentId) VALUES (?,?,?,?,?)",
	...
	"addBrpReward": (player) => {
        if (player.brpTime <= 0) return player.utils.error(`Ошибка начисления награды. Обратитесь к администрации!`);
		player.brpTime = 0;
		DB.Handle.query("UPDATE talrasha_character SET brpTime=? WHERE id=?", [0, player.sqlId]);
		player.utils.setDonate(player.account.donate + 100);
		player.utils.success(`Награда получена!`);
    },
	...
	"server::initPlayerSpawn": (player, value) => {
		if (player.demorgan > 0) {
			var pos = new mp.Vector3(SpawnInfo.demorgan.x, SpawnInfo.demorgan.y, SpawnInfo.demorgan.z);
			...
			if (player.demorganTimerId) {
				...
				player.health = 100;
				player.dimension = 0;
				return player.call("authCharacter.success");
			}
		...
		var talrashapos;
		let spawn = SpawnInfo.user_spawn[(0)]; // Приезжий_LS_1
		
		if(value == 0){
			if (player.enteredHouse) {
				var house = mp.houses.getBySqlId(player.enteredHouse);
				if (house) {
			...
			player.spawn(player.spawnPos);
			player.heading = 327;
			player.dimension = 0;
			
		}
		else if(value == 1){
			if (!player.houseId) return player.utils.error(`У вас нет дома!`);
			
			var house = mp.houses.getBySqlId(player.houseId);
			talrashapos = new mp.Vector3(house.x, house.y, house.z);
			talrashapos.h = house.h;
			player.spawn(talrashapos);
			player.heading = talrashapos.h;
			player.dimension = 0;
		}
		else if(value == 2){
			if (!player.faction) return player.utils.error(`Вы не состоите в какой либо организации!`);
			var faction = mp.factions.getBySqlId(player.faction);
            talrashapos = faction.position;
            talrashapos.h = faction.h;
			player.spawn(talrashapos);
			player.heading = talrashapos.h;
			player.dimension = 0;
		}
		else if(value == 3){
				talrashapos = new mp.Vector3(spawn.x, spawn.y, spawn.z);
				talrashapos.h = spawn.h;
				player.spawn(talrashapos);
				player.heading = talrashapos.h;
				player.dimension = 0;
		}
		player.call("authCharacter.success");
		player.call("chat.custom.push", [`Добро пожаловать на <a style="color: #FDD331">тестовый сервер!</a>`]);
		player.call("chat.custom.push", [`По всем вопросам: <a style="color: #FDD331">https://t.me/talrasha1</a>`]);
		//player.call('charChoosed.client');
		player.call("casino::create_tables", [ JSON.stringify(mp.casinoInfo) ]);
		DB.Handle.query(`SELECT online_wheel FROM talrasha_character WHERE id = ? LIMIT 1`, [player.sqlId], (e, result) => {
			user.set(player, 'online_wheel', result[0]['online_wheel']);
		});
    },
    ...
    "authCharacter": (player, characterIndex) => {
        ...
        if (character.ban > 0) {
            ...
            if (diff > 0) {
                ...
                DB.Handle.query("UPDATE talrasha_character SET ban=? WHERE id=?", [0, character.id]);
            } else {
        ...
        player.dimension = 0;
		//DB.Handle.query("UPDATE talrasha_character SET online_wheel=online_wheel+? WHERE id=?", [user.get(player, 'online_wheel') + 1, character.id]);
		...
		player.setTatoos(character.tatoos);

        DB.Handle.query("SELECT * FROM talrasha_character_overlay WHERE character_id = ?", [character.id], (e, result) => {
        ...
        player.call(`hudControl.setData`, [{
			...
			brpTime: player.brpTime,
			donate: player.account.donate
        }]);
		
		const { initCallZone } = require("../talrasha_module/talrasha_ghetto/talrasha_ghetto.js");
		initCallZone(player)
		const { initMarker } = require("../talrasha_module/talrasha_wzp/talrasha_wzp.js");
		initMarker(player)
		const { initMarkers } = require("../talrasha_module/talrasha_bizwar/talrasha_bizwar.js");
		initMarkers(player)

		player.call('ipad.init')
		characterInitOnline(player, 1)
		//initTeamPlayers(player)
		player.call('initHudCapture')
		player.call('charChoosed.client');
        //player.call("authCharacter.success");
        player.authTime = new Date().getTime();

        //начисление награды за промокоды
        DB.Handle.query(`SELECT * FROM talrasha_promocode WHERE character_id=? and reward_sum > 0`, [character.id], (e, result) => {
            ...
            if (result.length > 0) {
                ...
                DB.Handle.query("UPDATE talrasha_promocode SET reward_sum=0 WHERE character_id=?", [character.id]);
            }
        });
        ...
        if (character.promoId !== 0) {
            ...
            //DB.Handle.query(`SELECT * FROM talrasha_promocode_data WHERE regId=? and idcode=?`, [player.account.id, character.promoId], (e, result) => {
            DB.Handle.query(`SELECT c.reward_config,d.rewarded FROM talrasha_promocode_data as d LEFT JOIN talrasha_promocode as c ON  d.idcode = c.id WHERE d.regId=?`, [player.account.id], (e, result) => {
                if (result.length > 0) {
                    ...
                    if (streamerSumRevard > 0) {
                        DB.Handle.query("UPDATE talrasha_promocode SET reward_sum=reward_sum+? WHERE id=?", [streamerSumRevard, character.promoId]);
                    }
                    if (characterSumRevard > 0) {
                        DB.Handle.query("UPDATE talrasha_promocode_data SET rewarded=? WHERE idcode=? and regId=?", [JSON.stringify(dataRewardPromo), character.promoId, player.account.id]);
        ...
        if (player.warntime > 0) {
            ...
            if (diff > 0) {
                ...
                DB.Handle.query("UPDATE talrasha_character SET warn=? WHERE id=?", [player.warn, character.id]);
                if (player.warn > 0)
                    DB.Handle.query("UPDATE talrasha_character SET warntime=? WHERE id=?", [date.getTime() / 1000 + 15 * 24 * 60 * 60, character.id]);
                else
                    DB.Handle.query("UPDATE talrasha_character SET warntime=? WHERE id=?", [0, character.id]);
            } else {
		...
		player.call("loadCharacter.success");
    },
            ...
            for (var i = 0; i < player.characters.length; i++) {
                ...
                if (ch.name == name) {
                    if (ch.ban > 0) {
                      ...
                      DB.Handle.query(`UPDATE talrasha_character SET accountId=? WHERE id=?`, [-player.account.id, ch.id], () => {
                          player.utils.initChoiceCharacter();
    ...
    "characters.openSlot": (player, type) => {
        ...
        if (type == "achievements") {
            ...
            if (player.account.achievements_slot) return player.utils.error(`2 слот уже открыт!`);
            if (count < achievements) return player.utils.error(`Необходимо открыть 2 слот! (${count} из ${achievements})`);
            DB.Handle.query(`UPDATE talrasha_account SET achievements_slot=? WHERE id=?`,
                [1, player.account.id]);
            player.account.achievements_slot = true;
        } else if (type == "donate") {
            if (!player.account.achievements_slot) return player.utils.error(`Сначала разблокируйте 2 слот!`);
            if (player.account.donate_slot) return player.utils.error(`Слот за LRP уже открыт!`);
            var price = mp.economy["open_slot_donate_price"].value;
            if (player.account.donate < price) return player.utils.error(`Необходимо ${price} LRP!`);
            player.account.donate -= price;
            DB.Handle.query(`UPDATE talrasha_account SET donate_slot=?,donate=? WHERE id=?`,
...
function initDBParams(player, params) {
    ...
    player.appearance = {
        tatoos: params.tatoos
    }
	player.brpTime = params.brpTime;
	player.enteredHouse = params.enteredHouse;
	player.roulette = params.roulette ? JSON.parse(params.roulette) : [];
	player.phone = params.phone || "Отсутствует";
    player.spawnPos = {
        x: params.x,
        y: params.y,
        z: params.z,
	...
	player.call(`roulette.setPrizes`, [JSON.stringify(player.roulette)]);
...
function initPlayerUtils(player) {
	...
	player.utils.changeName = (name) => {
		player.name = name;
		player.utils.setLocalVar("name", name);
		DB.Handle.query("UPDATE talrasha_character SET name=? WHERE id=?", [player.name, player.sqlId]);
	};
    player.utils.setBankMoney = (newMoney) => {
        ...
        DB.Handle.query("UPDATE talrasha_character SET bank=? WHERE id=?", [player.bank, player.sqlId]);
    ...
    player.utils.leaveDemorgan = () => {
        ...
        player.demorgan = 0;
        DB.Handle.query("UPDATE talrasha_character SET demorgan=? WHERE id=?", [0, player.sqlId]);
    ...
    player.utils.stopVoiceMute = () => {
        player.vmute = 0;
        DB.Handle.query("UPDATE talrasha_character SET vmute=? WHERE id=?", [0, player.sqlId]);
	  ...
	  player.utils.stopMute = () => {
        player.mute = 0;
        DB.Handle.query("UPDATE talrasha_character SET mute=? WHERE id=?", [0, player.sqlId]);
	...
	player.utils.addRoulettePrize = (prize, count = 0) => {
        player.roulette.push({prize: prize, count: count})
		DB.Handle.query("UPDATE talrasha_character SET roulette=? WHERE id=?", [JSON.stringify(player.roulette), player.sqlId]);
    };
	player.utils.deleteRoulettePrize = (id) => {
		player.roulette.splice(id, 1);
		DB.Handle.query("UPDATE talrasha_character SET roulette=? WHERE id=?", [JSON.stringify(player.roulette), player.sqlId]);
    };
    player.utils.setDonate = (newDonate) => {
        player.account.donate = newDonate;
        player.utils.setLocalVar("donate", player.account.donate);
        DB.Handle.query("UPDATE talrasha_account SET donate = ? WHERE id=?", [player.account.donate, player.account.id]);
    };
    player.utils.setMoney = (newMoney, dontTouchInventory, callback) => {
        ...
        DB.Handle.query("UPDATE talrasha_character SET money=? WHERE id=?", [player.money, player.sqlId], () => {
            callback();
        });
    ...
    player.utils.setFaction = (faction, rank = 0) => {
        ...
        player.setVariable('factionNum', player.faction);
        player.utils.setLocalVar("faction", player.faction);
        player.utils.setLocalVar("factionRank", player.rank);
        DB.Handle.query("UPDATE talrasha_character SET `faction`=?,`rank`=? WHERE `id`=?", [player.faction, player.rank, player.sqlId]);
        if (player.faction > 0) {
            var faction = mp.factions.getBySqlId(player.faction);
    ...
    player.utils.setFactionRank = (rank = 1) => {
        ...
        if (player.faction && !rank) player.rank = 1;
        player.utils.setLocalVar("factionRank", player.rank);
		//debug(`player.utils.setFaction: ${player.rank} ${player.sqlId}`);
        DB.Handle.query("UPDATE talrasha_character SET `rank`=? WHERE `id`=?", [player.rank, player.sqlId]);
        if (player.faction > 0) {
            var faction = mp.factions.getBySqlId(player.faction);
            var rankName = mp.factions.getRankName(player.faction, player.rank);
            var rankPay = mp.factions.getRankPay(player.faction, player.rank);
    ...
    player.utils.setAdmin = (adminLevel) => {
        ...
        DB.Handle.query("UPDATE talrasha_character SET admin=? WHERE id=?", [player.admin, player.sqlId]);
    ...
    player.utils.setJob = (job) => {
        ...
        player.utils.setLocalVar("job", player.job);
        DB.Handle.query("UPDATE talrasha_character SET job=? WHERE id=?", [player.job, player.sqlId]);
    };
	player.utils.setSimcard = (number) => {
        player.phone = number
		player.call("phone.setPhoneNumber", [player.phone])
        DB.Handle.query("UPDATE talrasha_character SET phone = ? WHERE id = ?", [number, player.sqlId])
    };
    player.utils.setJobSkills = (job, exp) => {
        job = Math.clamp(job, 1, mp.jobs.length - 1);
        if (!player.jobSkills[job - 1]) {
            DB.Handle.query("INSERT INTO talrasha_job_skill (playerId, jobId, exp) VALUES (?,?,?)",
                [player.sqlId, job, exp]);
        } else if (!exp) {
            DB.Handle.query("DELETE FROM talrasha_job_skill WHERE playerId=? AND jobId=?", [player.sqlId, job]);
        } else {
            DB.Handle.query("UPDATE talrasha_job_skill SET exp=? WHERE playerId=? AND jobId=?",
    ...
    player.utils.setWanted = (newWanted) => {
        ...
        if (player.wanted != newWanted) DB.Handle.query("UPDATE talrasha_character SET wanted=?,law=?,crimes=? WHERE id=?", [newWanted, player.law, player.crimes, player.sqlId]);
        ...
        player.wantedTimer = timerId = setInterval(() => {
            try {
                ...
                DB.Handle.query("UPDATE talrasha_character SET wanted=? WHERE id=?", [rec.wanted, rec.sqlId]);
    ...
    player.utils.setArrestTime = (arrestSeconds) => {
        ...
        DB.Handle.query("UPDATE talrasha_character SET arrestTime=? WHERE id=?", [arrestSeconds, player.sqlId]);
    };
    player.utils.doArrest = (cellIndex, arrestSeconds, isSpawn) => {
        ...
        player.arrestTimerId = timerId = setTimeout(() => {
            try {
                ...
                DB.Handle.query("UPDATE talrasha_character SET arrestCell=? WHERE id=?", [0, rec.sqlId]);
    ...
    player.utils.clearArrest = () => {
        ...
        DB.Handle.query("UPDATE talrasha_character SET arrestCell=? WHERE id=?", [0, player.sqlId]);
    };
	...
	player.utils.saveEnteredHouse = (save, id) => {
        if (save) {
			player.inHouse = id;
			DB.Handle.query(`UPDATE talrasha_character SET enteredHouse = ? WHERE id = ?`,
				[player.inHouse, player.sqlId]);
		}
		else {
			delete player.inHouse;
			DB.Handle.query(`UPDATE talrasha_character SET enteredHouse = 0 WHERE id = ?`,
				[player.sqlId]);
		}
    };
    ...
    player.utils.removeBiz = (biz) => {
        player.call(`playerMenu.removeBiz`, [biz.sqlId]);
    };
    ...
    player.utils.setSatiety = (newSatiety) => {
        if (player.admin < 1) {
            ...
            DB.Handle.query("UPDATE talrasha_character SET satiety=? WHERE id=?", [player.satiety, player.sqlId]);
            player.call(`inventory.setSatiety`, [player.satiety]);
    ...
    player.utils.setThirst = (newThirst) => {
        if (player.admin < 1) {
            ...
            DB.Handle.query("UPDATE talrasha_character SET thirst=? WHERE id=?", [player.thirst, player.sqlId]);
         player.call(`inventory.setThirst`, [player.thirst]);
...
function initLocalParams(player) {
    ...
    if (houses.length != 0) {
		...
		var pos = new mp.Vector3(houses[0].x, houses[0].y, houses[0].z);
		var posgarage = new mp.Vector3(houses[0].garageX, houses[0].garageY, houses[0].garageZ);
		player.call("client::addHouseBlip", [true, pos]) 
		player.call("client::addGarageMarker", [true, posgarage]) 
		houses.garageMarker = mp.createHouseGarageMarker(enterHouse);
    } else {
    ...
    DB.Handle.query(`SELECT * FROM talrasha_character WHERE accountId = ?`,
    ...
    DB.Handle.query(`SELECT * FROM talrasha_unitpay_payment WHERE account = ? ORDER BY id DESC LIMIT 5`,
    [player.account.login], (e, result) => {
        if (result.length >= 0) {
            ...
            /*if(player.account.done == 0) {
                ...
                DB.Handle.query("UPDATE talrasha_account SET donate = ? WHERE login = ?", [player.account.donate, player.account.login]);
                DB.Handle.query("UPDATE talrasha_account SET done = 1 WHERE login = ?", [player.account.login]);
            }*/
	...
	player.utils.setLocalVar("regDate", player.account.regDate);
	player.utils.setLocalVar("lastDate", player.account.lastDate);
	...
	player.setVariable("sqlId", player.sqlId);
	player.utils.setLocalVar("id", player.id);
    ...
    player.utils.setLocalVar("bank", player.bank);
	player.utils.setLocalVar("phone", player.phone); 
    player.call(`playerMenu.setSpawn`, [player.spawnPoint, 'server']);
    player.call(`playerMenu.setHouseId`, [player.houseId, 'server']);
...
function initFinesCount(player) {
    DB.Handle.query(`SELECT * FROM talrasha_fine WHERE recipient=?`, [player.sqlId], (e, result) => {
...
function initJobSkills(player) {
    ...
    DB.Handle.query("SELECT * FROM talrasha_job_skill WHERE playerId=?", player.sqlId, (e, result) => {
        for (var i = 0; i < result.length; i++) {
	...
	/*player.carIds = [];
    player.cars = [];
    DB.Handle.query("SELECT * FROM talrasha_vehicle WHERE owner=?", [2000 + player.sqlId], async (e, result) => {
        for (var i = 0; i < result.length; i++) {
            var v = result[i];
            player.cars.push({
                name: v.model,
				sqlId: v.id,
				price: v.price,
				plate: v.numberplate ? v.numberplate : ''
            });
        }
...
function initPlayerPhoneData (player) {   
    player.setVariable("phoneContacts", JSON.stringify(mp.phone_contacts.getByCharacterSqlId(player.sqlId)))
    player.setVariable("phoneCalls", JSON.stringify(mp.phone_calls.getByCharacterSqlId(player.sqlId)))  
	player.setVariable("phoneMessages", JSON.stringify(mp.phone_messages.getByPhoneNumber(player.phone)))
	player.call("phone.setPhoneNumber", [player.phone])
	DB.Handle.query("SELECT * FROM talrasha_log_bank WHERE playerId=? ORDER BY id DESC LIMIT 30", [player.sqlId], (e, result) => {
		for (var i = 0; i < result.length; i++) {
		...
		player.call("phone.setLogs", [JSON.stringify(result)]);
	});
}
