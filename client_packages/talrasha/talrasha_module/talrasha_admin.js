...
mp.events.add('admin.control.freezecar', (time) => {
    if (AdminInfo.freezecar_timer !== undefined) {
        mp.vehicles.freezePosition(false);
        clearTimeout(AdminInfo.freezecar_timer);
        delete AdminInfo.freezecar_timer;
    }
    if (time === 0) return;
    if (AdminInfo.freezecar_timer === undefined) {
        mp.vehicles.freezePosition(true);
        let parsedNum = time * 1000;
        AdminInfo.freezecar_timer = setTimeout(() => {
            mp.vehicles.freezePosition(false);
            mp.events.callRemote("delete.player.admin.freeze");
        }, parsedNum);
    }
