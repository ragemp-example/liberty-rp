...
mp.events.add('clear.taxi.filtration', (type) => {
  JobTaxi.blacklist = [];
  mp.events.call(`nSuccess`, `Вы сбросили черный список!`);
});
mp.events.add('add.taxi.filtration', (name) => {
  if (!JobTaxi.blacklist.includes(name)) {
    JobTaxi.blacklist.push(name);
    mp.events.call(`nSuccess`, 'Игрок добавлен в черный список и больше не будет отображаться.');
  }
});
mp.events.add('show.taxi.menu', (name) => {
  if (!CEF) {
	  setTimeout(() => {
		CEF = mp.browsers.new("package://talrasha/talrasha_module/talrasha_job/talrasha_taxi/talrasha_ui/talrasha_panel/index.html");
		setCursor(true);
		}, 100);
	mp.events.callRemote('accept.taxi.day');
  }
});
...
mp.events.add('cant.finish.taxi.job', () => {
  mp.events.call(`nWarning`, `Вы не завершили вызов`);
});
...
mp.keys.bind(88, false, function () { // Numpad 0
...
mp.events.add('talrashataxiuval', () => {
	setCursor(false);
...
mp.events.add('get.taxi.waypoint', (target) => {
   ...
   if (pos === undefined) {
	 mp.events.call(`nWarning`, `Вы не установили метку на карте`);
     return;
   }
...
mp.keys.bind(0x45, false, function () { // E key
	if (keyDownE !== undefined) {
		if (mp.game.gameplay.getDistanceBetweenCoords(mp.players.local.position.x, mp.players.local.position.y, mp.players.local.position.z, 903.48, -173.77, 74.08, true) < 4){
      mp.gui.cursor.show(true, true);
      if (keyDownE === true) {
          mp.events.call("choiceMenu.show", "accept_job_taxi", {name: "уволиться из Таксопарка?"});
		 // mp.events.call(`nOffer`, `Вы хотите уволиться из таксопарка?`);
      } else {
					mp.events.call("choiceMenu.show", "accept_job_taxi", {name: "устроиться в Таксопарк?"});
					//mp.events.call(`nOffer`, `Вы хотите устроиться в таксопарк?`);
      }
		}
	if (mp.game.gameplay.getDistanceBetweenCoords(mp.players.local.position.x, mp.players.local.position.y, mp.players.local.position.z, -1536.58, -395.00, 42.04, true) < 4){
      mp.gui.cursor.show(true, true);
      if (keyDownE === true) {
          mp.events.call("choiceMenu.show", "accept_job_taxi", {name: "уволиться из Таксопарка?"});
		 // mp.events.call(`nOffer`, `Вы хотите уволиться из таксопарка?`);
	...
	if (mp.game.gameplay.getDistanceBetweenCoords(mp.players.local.position.x, mp.players.local.position.y, mp.players.local.position.z, 1945.39, 3764.31, 32.21, true) < 4){
      mp.gui.cursor.show(true, true);
      if (keyDownE === true) {
          mp.events.call("choiceMenu.show", "accept_job_taxi", {name: "уволиться из Таксопарка?"});
		  mp.events.call(`nOffer`, `Вы хотите уволиться из таксопарка?`);
