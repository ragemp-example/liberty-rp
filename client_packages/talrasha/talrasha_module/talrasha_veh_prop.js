...
startMileageCounter = function(menu) {
    ...
    mileageTimer = setInterval(() => {
        ...
        menu.execute(`window.events.call('carSystem', { mileage: ${mileage}, event: 'MileageHandler' })`);
...
exports = (menu) => {
    ...
    mp.events.add('playerEnterVehicle', (vehicle, seat) => {
        //menu.execute(`window.events.call('carSystem', { enable: ${true}, event: 'vehEnable' })`);
    });
	
	mp.keys.bind(0x4C, false,  () => { 
		 if (mp.bindlocker()) return;
		mp.events.callRemote("server::carOpeningSwitch")
	});
	...
	mp.events.addDataHandler("isLockCar", (entity, value) => {
        let dist = vdist(mp.players.local.position, entity.position)
        if(dist > 15) return
        if(value){
            entity.setLights(2); 
            mp.events.call("enableSound", "talrasha_lock.mp3", 0.1)
            setTimeout(() => { 
                entity.setLights(0); 
                setTimeout(() => { 
                    entity.setLights(2); 
    ...
    mp.events.addDataHandler("leftSignal", (entity) => {
        ...
        if (player.vehicle && entity.remoteId == player.vehicle.remoteId) {
            ...
            if (engine) {
                if (!left || !right) {
                    menu.execute(`window.events.call('carSystem', { left: ${left}, event: 'LeftSignalHandler' })`);
                    menu.execute(`window.events.call('carSystem', { right: ${right}, event: 'RightSignalHandler' })`);
                    menu.execute(`window.events.call('carSystem', { emergency: ${false}, event: 'EmergencyHandler' })`);
                } else {
                    menu.execute(`window.events.call('carSystem', { emergency: ${true}, event: 'EmergencyHandler' })`);
                }
            } else {
                menu.execute(`window.events.call('carSystem', { left: ${false}, event: 'LeftSignalHandler' })`);
                menu.execute(`window.events.call('carSystem', { right: ${false}, event: 'RightSignalHandler' })`);
                menu.execute(`window.events.call('carSystem', { emergency: ${false}, event: 'EmergencyHandler' })`);
            }
        }
    });
        ...
        if (player.vehicle && player.vehicle.remoteId == entity.remoteId) {
            ...
            if (engine) {
                if (!left || !right) {
                    ...
                    menu.execute(`window.events.call('carSystem', { engine: ${true}, event: 'engineStatus' })`);
                    menu.execute(`window.events.call('carSystem', { accumulator: ${true}, event: 'accumulatorStatus' })`);
                    menu.execute(`window.events.call('carSystem', { oil: ${true}, event: 'OilBrokenHandler' })`);
                } else {
                    menu.execute(`window.events.call('carSystem', { emergency: ${true}, event: 'EmergencyHandler' })`);
                }
            } else {
                ...
                menu.execute(`window.events.call('carSystem', { engine: ${false}, event: 'engineStatus' })`);
                menu.execute(`window.events.call('carSystem', { accumulator: ${false}, event: 'accumulatorStatus' })`);
                menu.execute(`window.events.call('carSystem', { oil: ${false}, event: 'OilBrokenHandler' })`);
            }
        }
    });
    ...
    mp.events.addDataHandler("engineBroken", (entity) => {
        ...
        menu.execute(`window.events.call('carSystem', { engine: ${engineBroken}, event: 'engineStatus' })`);
    });

    mp.events.addDataHandler("oilBroken", (entity) => {
        if (entity.getPedInSeat(-1) != player.handle) return;
        menu.execute(`window.events.call('carSystem', { oil: ${oilBroken}, event: 'OilBrokenHandler' })`);
    });

    mp.events.addDataHandler("accumulatorBroken", (entity) => {
        if (entity.getPedInSeat(-1) != player.handle) return;
       menu.execute(`window.events.call('carSystem', { accumulator: ${accumulatorBroken}, event: 'accumulatorStatus' })`);
    ...
    mp.events.add("render", () => {
        if (player.vehicle && player.vehicle.getPedInSeat(-1) == player.handle) {
            ...
            if (data.lastLocked != locked) {
                menu.execute(`window.events.call('carSystem', { locked: ${locked}, event: 'LockedHandler' })`);
            ...
            if (engine) {
                ...
                if (data.lastLights != lightsType) {
                    menu.execute(`window.events.call('carSystem', { lights: ${lightsType}, event: 'LightsHandler' })`);
                    data.lastLights = lightsType;
                }
            } else if (data.lastLights != 0) {
                menu.execute(`window.events.call('carSystem', { lights: ${0}, event: 'LightsHandler' })`);
                data.lastLights = 0;
            }

            menu.execute(`window.events.call('carSystem', { oil: ${veh.oilBroken}, event: 'OilBrokenHandler' })`);
            menu.execute(`window.events.call('carSystem', { accumulator: ${veh.accumulatorBroken}, event: 'accumulatorStatus' })`);
            if (!player.vehicle.getIsEngineRunning()) {
                var test = { velocity: velocity, fuel: fuel, rpm: 0, gear: veh.gear };
                menu.execute(`window.events.call('carSystem', { data: ${JSON.stringify(test)}, event: 'VehPropHandler' })`);
            } else {
            ...
            menu.execute(`window.events.call('carSystem', { right: ${false}, event: 'RightSignalHandler' })`);
            menu.execute(`window.events.call('carSystem', { emergency: ${false}, event: 'EmergencyHandler' })`);
            menu.execute(`window.events.call('carSystem', { locked: ${true}, event: 'LockedHandler' })`);
            menu.execute(`window.events.call('carSystem', { belt: ${true}, event: 'BeltHandler' })`);
            menu.execute(`window.events.call('carSystem', { oil: ${false}, event: 'OilBrokenHandler' })`);
            menu.execute(`window.events.call('carSystem', { accumulator: ${false}, event: 'accumulatorStatus' })`);
            menu.execute(`window.events.call('carSystem', { engine: ${false}, event: 'engineStatus' })`);
            menu.execute(`window.events.call('carSystem', { mileage: ${0}, event: 'MileageHandler' })`);
            menu.execute(`window.events.call('carSystem', { enable: ${false}, event: 'vehEnable' })`);
            menu.execute(`window.events.call('carSystem', { belt: ${true}, event: 'BeltHandler' })`);
            menu.execute(`window.events.call('carSystem', { lights: ${0}, event: 'LightsHandler' })`);
            player.belt = false;
            vehPropActive = false;
