...
const { createPed } = require("talrasha/talrasha_module/talrasha_helper/talrasha_ped_helper.js");

const localPlayer = mp.players.local;
const info = {
	spawn: { position: new mp.Vector3(-78.68, -810.60, 243.39), heading: 0 },
...
mp.events.add("character_creation::init", async () => {
	...
	mp.events.call("castomMenu.show");
...
mp.events.add("character_creation::stop", () => {
	menu.execute(`castomApp.closeMenu()`);
...
mp.events.add("castom.toggleGender", () => {
	toggleGender();
});

mp.events.add("castom.chooseMother", (valueIndex) => {
	editData.motherIndex = valueIndex;
	setPedsCurrentHeadBlendData();
	//mp.events.call('TalRashaNotify', 3, 9, `[Мать] - ${valueIndex}`, 3000);
});

mp.events.add("castom.chooseFather", (valueIndex) => {
	editData.fatherIndex = valueIndex;
	setPedsCurrentHeadBlendData();
	//mp.events.call('TalRashaNotify', 3, 9, `[Отец] - ${valueIndex}`, 3000);
});

mp.events.add("castom.chooseAppear", (valueIndex) => {
	editData.shapeMix = valueIndex / 100;
	setPedsCurrentHeadBlendData();
	//mp.events.call('TalRashaNotify', 3, 9, `[Сходство] - ${valueIndex}% | ${valueIndex / 100`, 3000);
});

mp.events.add("castom.chooseBodyMix", (valueIndex) => {
	editData.skinMix = valueIndex / 100;
	setPedsCurrentHeadBlendData();
	//mp.events.call('TalRashaNotify', 3, 9, `[Кожа] - ${valueIndex}% | ${valueIndex / 100`, 3000);
});
...
mp.events.add("castom.chooseSpecifications", (valueIndex, itemIndex) => {
	const value = valueIndex * 0.02 - 1;
	editData.faceFeatures[itemIndex] = value;
	setPedsFaceFeature(itemIndex, value);
	// mp.events.call('TalRashaNotify', 3, 9, `[Лицо] - ${valueIndex}% | ${value} | ${itemIndex}`, 3000);
});

mp.events.add("castom.chooseHair", (valueIndex) => {
	editData.appearance.hairIndex[editData.selectedGender] = valueIndex;
	// mp.events.call('TalRashaNotify', 3, 9, `Волосы: [" + valueIndex + "]`, 3000);
	setPedCurrentHair();
});
mp.events.add("castom.chooseHairColor", (valueIndex) => {
	editData.appearance.hairColorIndex = valueIndex;
	// mp.events.call('TalRashaNotify', 3, 9, `Волосы | ЦВЕТ: [" + valueIndex + "]`, 3000);
	setPedsCurrentHairColor();
});
mp.events.add("castom.chooseEyeColor", (valueIndex) => {
	editData.appearance.eyeColorIndex = valueIndex;
	setPedsCurrentEyeColor();
});
mp.events.add("castom.chooseFaceComponentColor", (valueIndex, itemName) => {
	...
	editData.appearance.headOverlays[headOverlayId].colorIndex = valueIndex;
	// mp.events.call('TalRashaNotify', 3, 9, `ЦВЕТ - [${itemName}] | [${valueIndex}]`, 3000);
	setPedsCurrentHeadOverlayColor(headOverlayId);
});
mp.events.add("castom.chooseFaceComponent", (valueIndex, itemName) => {
	const headOverlayId = getHeadOverlayIdByItemName(itemName);
	...
	// mp.events.call('TalRashaNotify', 3, 9, `[${itemName}] | [${valueIndex}]`, 3000);
	setPedsCurrentHeadOverlay(headOverlayId);
});

mp.events.add("castom.chooseClothes", (valueIndex, itemName) => {
	if (itemName === "tops") {
...
mp.events.add("castom.focus", (menu) => {
	 focusCastomMenu(menu)
});

mp.events.add("castom:sliderValueChanged", (val) => {
	for (const ped of info.peds) {
		ped.setHeading(val);
	}
});

mp.events.add("castom.clearMenu", (menu) => {
	 clearCastomMenu(menu);
});

function focusCastomMenu(menu) {
	switch (menu) {
		case "character":
			mp.events.call("focusOnPlayer", localPlayer.position, -10);
			break;
...
exports = (m) => {
	menu = m;
	mp.events.add("castomMenu.show", () => {
			m.execute(`castomApp.openMenu()`);
	});
}