exports = (menu) => {
	...
	const loadingPrompt = require("talrasha/talrasha_module/talrasha_helper/talrasha_loading_prompt.js");
	const { getVehicleCharacteristics } = require("talrasha/talrasha_module/talrasha_helper/talrasha_vehicle_helper.js");
	const { escapeHtml } = require("talrasha/talrasha_module/talrasha_helper/talrasha_string_helper.js");
	...
	mp.events.add("ls_customs::start", (driverId, vehicleId, placeId, vehPrice) => {
		...
		if (driverId === mp.players.local.remoteId) {
			if (place.waypointRecord) {
				...
				if (place.noWaypointStopPosition) {
					vehicle.position = place.noWaypointStopPosition.position;
					vehicle.setHeading(place.noWaypointStopPosition.heading);
				}
				setTimeout(() => {
					fadeScreen(false, 50);
					camera.setActive(true);
	...
	mp.events.add("ls_customs::open_doors", (placeIndex) => {
	...
	function startEnterInteraction(placeIndex) {
		mp.events.callRemote("ls_customs::open_lsc_doors", placeIndex);
	}
	...
	mp.events.add("selectMenu.itemSelected", async (menuName, item, itemValue, itemIndex) => {
		...
		if (menuName === "ls_customs_repair") { 
			if (itemName === "Ремонт") {
				...
				await checkPrice("repair", 0, 0, currentRepairPrice)
			...
			if (itemName === "Выход") {
				...
				mp.events.call("selectMenuTalRasha.show", "ls_customs_headlightsMenu");
			} else if (itemName === "Клаксоны") {
				currentHornValue = mp.players.local.vehicle.getMod(14);
				currentHornType = -1;
				mp.events.call("selectMenuTalRasha.show", "ls_customs_hornsMenu");
			} else if (itemName === "Номера") {
				showNumberPlateMenu();
			} else if (itemName === "Окраска") {
				mp.events.call("selectMenuTalRasha.show", "ls_customs_colorMenu");
			} else if (itemName === "Колёса") {
				mp.events.call("selectMenuTalRasha.show", "ls_customs_wheelsMenu");
			...
			/*if (!await checkPrice("mods", currentModType, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("mods", currentModType, itemIndex, undefined, itemIndex, itemName)
			...
			if (itemName === "Фары") {
				...
				mp.events.call("selectMenuTalRasha.show", "ls_customs_neonMenu");
			}
			...
			/*if (!await checkPrice("mods", 22, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("mods", 22, itemIndex, undefined, itemIndex, itemName)
			...
			/*if (!await checkPrice("neonPosition", 0, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("neonPosition", 0, itemIndex, undefined, itemIndex, itemName)
			...
			/*if (!await checkPrice("neonColor", 0, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("neonColor", 0, itemIndex, undefined, itemIndex, itemName)
			...
			/*if (!await checkPrice("mods", 14, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("mods", 14, itemIndex, undefined, itemIndex, itemName)
			...
			/*if (!await checkPrice("numberPlateType", 0, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("numberPlateType", 0, itemIndex, undefined, itemIndex, itemName)
			...
			mp.events.call("selectMenuTalRasha.show", "ls_customs_colorGroup");
			...
			if (isCurrentColorPrimary) {
				...
				/*if (!await checkPrice("color", 0, currentColorGroup, undefined, itemIndex, itemName)) {
					return;
				}*/
				
				await checkPrice("color", 0, currentColorGroup, undefined, itemIndex, itemName)
			...
			if (itemName === "Колёса") {
				...
				mp.events.call("selectMenuTalRasha.show", `ls_customs_wheels_${wheelMenu}`, selectedIndex);
			} else if (itemName === "Цвет колёс") {
				showWheelsColorMenu();
			} else if (itemName === "Шины") {
				mp.events.call("selectMenuTalRasha.show", `ls_customs_wheels_tiresMenu`);
			}
			...
			mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_typeMenu");
			...
			/*if (!await checkPrice(priceKey, 0, priceIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice(priceKey, 0, priceIndex, undefined, itemIndex, itemName)
			...
			/*if (!await checkPrice("wheelColor", 0, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("wheelColor", 0, itemIndex, undefined, itemIndex, itemName)
			...
			/*if (!await checkPrice("tiresDesign", 0, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("tiresDesign", 0, itemIndex, undefined, itemIndex, itemName)
			...
			/*if (!await checkPrice("tiresBurst", 0, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("tiresBurst", 0, itemIndex, undefined, itemIndex, itemName)
			...
			/*if (!await checkPrice("tiresSmokeColor", 0, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("tiresSmokeColor", 0, itemIndex, undefined, itemIndex, itemName)
			...
			/*if (!await checkPrice("mods", 18, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("mods", 18, itemIndex, undefined, itemIndex, itemName)
			...
			/*if (!await checkPrice("windowTint", 0, itemIndex, undefined, itemIndex, itemName)) {
				return;
			}*/
			
			await checkPrice("windowTint", 0, itemIndex, undefined, itemIndex, itemName)
	...
	mp.events.add("selectMenu.backspacePressed", (menuName) => {
		if (menuName === "ls_customs_concreteMod") {
			mp.events.call("selectMenuTalRasha.show", "ls_customs_main", mainMenuIndex);
			...
			if (mp.game.vehicle.isThisModelACar(mp.players.local.vehicle.model)) {
				mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_carMenu", getWheelTypeIndexByValue(selectedWheelType));
			} else {
				mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_motoMenu");
			}
		} else if (menuName === "ls_customs_wheels_concreteMenu") {
	...
	function showMenu(vehicle, forceMain = false) {
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_main");
	}
	...
	function showConcreteMenu(modType) {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_concreteMod", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_concreteMod", selectedIndex);
	}

	function showHeadlightsMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_headlights", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_headlights", isXenonEnabled ? 1 : 0);
		vehicle.setLights(2);
	}

	function showNeonPositionMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_neonPosition", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_neonPosition", getIndexByNeonValues(neonLightStates));
	}

	function showNeonColorMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_neonColor", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_neonColor", getNeonColorIndexByColor(colorObj.r, colorObj.g, colorObj.b));
	}

	function showHornsMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_hornsConcrete", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_hornsConcrete", selectedIndex);
	}

	function showNumberPlateMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_numberPlate", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_numberPlate", getNumberPlateIndexByValue(currentNumberPlateType));
	}

	function showConcreteColorMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_colorConcrete", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_colorConcrete", selectedIndex);
	...
	function showWheelsConcreteMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_concreteMenu", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_concreteMenu", selectedIndex);
	}

	function showTurboMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_turbo", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_turbo", mp.players.local.vehicle.isToggleModOn(18) ? 1 : 0);
	}

	function showWheelsColorMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_colors", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_colors", selectedIndex);
	}

	function showTiresDesignMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_tiresDesign", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_tiresDesign", mp.players.local.vehicle.getModVariation(23) ? 1 : 0);
	}

	function showTiresUpgradeMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_tiresUpgrade", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_tiresUpgrade");
	}

	function showTiresSmokeColorMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_tiresSmokeColor", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_wheels_tiresSmokeColor", selectedIndex);
	}

	function showWindowTintMenu() {
		if (checkPriceMenuItemIndex !== undefined) {
			return mp.events.call("selectMenuTalRasha.show", "ls_customs_windows", checkPriceMenuItemIndex);
		}
		...
		mp.events.call("selectMenuTalRasha.show", "ls_customs_windows", getWindowTintIndexByValue(currentWindowTint));
	}
	...
	function getModPrice(modType, index, subKey = "mods") {
		...
		else return Math.round(price * vehiclePrice);
	}
	...
	function showRepairMenu(price) {
		var customPrice = customsInfo.prices.repair
		
		if (customPrice > 1) {
			currentRepairPrice = customPrice;
		}
		else {
		...
		const items = [
			{
				text: "Ремонт",
				values: [currentRepairPrice]
			},
			{ text: "Выход" }
		];

		mp.events.call("selectMenu.setSpecialItems", "ls_customs_repair", items);
		mp.events.call("selectMenuTalRasha.show", "ls_customs_repair");
	}
	...
	function showSpecMenu(state) {
		if (state) {
			...
			menu.execute(`window.events.call('carSystem', {specMenu: [${characteristics.join(',')}], event: 'specMenu' })`);
		}

		menu.execute(`window.events.call('carSystem', {enable: ${state}, event: 'enable' })`);
	}

	function showModificationSetMessage(itemName) {
		mp.events.call(`nSuccess`, `Модификация ${itemName} успешно установлена!`, true);
	...
	function checkPrice(keyItem, modType, itemIndex = 0, repairPrice = false, currentIndex = 0, itemname) {
		...
		return new Promise((resolve, reject) => {
			...
			mp.events.callRemote("ls_customs::checkPrice", keyItem, modType, itemIndex, repairPrice, itemname);
		});
	}
