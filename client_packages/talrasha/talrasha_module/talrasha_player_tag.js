...
mp.events.add("familiar.clear", (name) => {
	mp.storage.data.familiar = {};
	
	mp.players.forEach(player => {
		if (player.isFamiliar) delete player.isFamiliar;
	});
});
...
mp.events.add('render', (nametags) => {
    ...
    nametags.forEach(nametag => {
        ...
        if (
			...
			var id = player.getVariable('sqlId');
			
            var username = (mp.clientStorage.admin) ? `${player.name} ${id} (${player.remoteId})` : `${id} (${player.remoteId})`;
...
function getNicknameColor(player) {
    ...
    if (admin >= 2 && admin < 4) return [0, 250, 154];
    if (admin >= 4 && admin < 6) return [255, 0, 0];
    if (admin >= 6 && admin < 8) return [255, 0, 0];
    if (admin == 8) return [157, 129, 186];
    if (admin == 9) return [255, 207, 64];
    if (admin >= 10) return [0, 0, 0];
    return [255, 255, 255];
}
