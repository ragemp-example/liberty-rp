$(document).ready(function() {
    ...
    window.documentsAPI = {
        ...
        showGov: (enable, data) => {
            ...
            if (enable) {
                inventoryAPI.show(false);
                $(`#GovPassport .name`).text(data.Name);
                $(`#GovPassport .id`).text(getPaddingNumber(data.ID.toString(), 5));
                $(`#GovPassport .rank`).text(data.Rank);
                $(`#GovPassport`).show("fast");

                promptAPI.showByName("documents_help");
                $(document).unbind("keydown", documentsAPI.eHandler);
                $(document).keydown(documentsAPI.eHandler);
            } else {
                $(`#GovPassport`).hide("fast");
            }
        },
        hide: () => {
            $(`#documents`).hide("fast");
        ...
        eHandler: (e) => {
            if (e.keyCode == 69 || e.keyCode == 27 ) { // E / esc
    ...
    function initLicenses(data) {
        ...
        $(`#documents #licenses .lsex`).text(data.sex);
        $(`#documents #licenses .lfName`).text(data.name.split(" ")[0]);
        $(`#documents #licenses .lsName`).text(data.name.split(" ")[1]);
        $(`#documents #licenses .lage`).text(age);
        $(`#documents #licenses .lid`).text(getCorrectId(data.id));
    }
    ...
    function initSelectorButtons() {
        ...
        $('#licenses_text').on("click", function() {
            if ($('#licenses_text').text().trim() === "Развернуть страницу...") {
                $(licenses).css({
                    'background-image': 'url(./talrasha_image/talrasha_document/talrasha_license_bg2.png)',
                    'width': '640px;',
                    'height': '491px'
                });
                /* $(licenses).animate({
                ...
                $(licenses).css({
                    'background-image': 'url(./talrasha_image/talrasha_document/talrasha_license_bg.png)',
                    'width': '640px;',
                    'height': '340px'
                });
                /* $(licenses).animate({
