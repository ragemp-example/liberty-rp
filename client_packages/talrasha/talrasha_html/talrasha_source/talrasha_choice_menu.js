$(document).ready(() => {
    ...
    var menus = {
		...
		"buy_wardrobe_confirm": {
            text: "Вы действительно хотите купить шкаф за 0$?",
            on: (values) => {
                houseMenu.__vue__.exitMenu();
                menus["buy_wardrobe_confirm"].text = `Вы действительно хотите купить шкаф за $${values.price}.`;
            },
            yes: () => {
                houseMenu.__vue__.exitMenu();
                mp.trigger("events.callRemote", "houseInteraction.wardrobeBuyed");
            },
            no: () => {}
        },
        ...
        "accept_respawn": {
            text: "Желаете вызвать врача?",
            yes: () => {
                mp.trigger("setDeathTimer", true);
            },
            no: () => {
				mp.trigger("setDeathTimer", false);
			},
			time: () => {
				mp.trigger("setDeathTimer", false);
			},
		...
		"sellcar_confirm": {
            ...
            on: (values) => {
                menus["sellcar_confirm"].text = `Вы действительно хотите продать ${values.name} за ${values.price}?`;
				menus["sellcar_confirm"].sqlId = values.sqlId;
            },
            yes: () => {
				var sqlId = menus["sellcar_confirm"].sqlId;
                mp.trigger("events.callRemote", "sell.veh.sell", sqlId);
            },
            no: () => {
        ...
        "accept_fix_car": {
            text: "Вы уверены, что хотите доставить транспорт за 3000$?",
            on: (sqlId) => {
                menus["accept_fix_car"].text = `Вы уверены, что хотите доставить транспорт за 3000$?`;
		...
		"accept_sell_car": {
            text: "Вы уверены, что хотите продать авто за 0$?",
            on: (data) => {
				console.log(data)
                menus["accept_sell_car"].text = `Вы уверены, что хотите продать ${data.name} за ${parseInt(data.price/2)}$?`;
                menus["accept_sell_car"].sqlid = data.sqlId;
            },
            yes: () => {
                var sqlid = menus["accept_sell_car"].sqlid;
                mp.trigger("events.callRemote", "sell.veh.playerMenu", sqlid);
            },
            no: () => {
    ...
    window.choiceMenuAPI = {
        show: (name, values) => {
            ...
            timerId = setInterval(() => {
                ...
                $(".choiceMenu .cancel").text(`Закрыть (${times}с)`);
                if (times < 1) {
					if (currentChoiceMenu.time) {
						currentChoiceMenu.time()
					}
                    clearInterval(timerId);
                    choiceMenuAPI.hide();
                }
