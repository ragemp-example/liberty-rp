	...
	window.castomApp = new Vue({
		...
		methods: {
			openMenu() {
				...
				//$("#castomMenu .talrashabgcastom").css("display", "block");
				$("#castomMenu .customizer-wrapper").css("display", "flex");
			...
			update(menu, inc, val) {
				switch (menu) {
					case "specifications":
						this.specifications[inc] = val;
						Vue.set(castomApp.specifications, inc, val);
						mp.trigger("castom.chooseSpecifications", val, inc);
						break;
					case "parents appearence":
						this.parents.appear = inc;
						mp.trigger("castom.chooseAppear", inc);
						break;
					case "parents body":
						this.parents.body = inc;
						mp.trigger("castom.chooseBodyMix", inc);
						break;
				}	
			...
			control(menu, status, inc = false) {
				...
				switch (menu) {
					case "parents mothers":
						...
						$(`#castomMenu #parents .parents .parent.parent-1`).css(`backgroundImage`, `url(talrasha_image/talrasha_customization/talrasha_parent/talrasha_female_${index}.png)`);
						break;
					case "parents fathers":
						...
						$(`#castomMenu #parents .parents .parent.parent-2`).css(`backgroundImage`, `url(talrasha_image/talrasha_customization/talrasha_parent/talrasha_male_${index}.png)`);
						break;
					case "parents appearence":
			...
			clear(current) {
				...
				switch (current) {
					...
					case "parents":
						...
						$(`#castomMenu #parents .parents .parent.parent-1`).css(`backgroundImage`, `url(talrasha_image/talrasha_customization/talrasha_parent/talrasha_female_0.png)`);
						$(`#castomMenu #parents .parents .parent.parent-2`).css(`backgroundImage`, `url(talrasha_image/talrasha_customization/talrasha_parent/talrasha_male_0.png)`);
			...
			closeMenu() {
				$("#castomMenu .customizer-wrapper").css("display", "none");
				$("#castomMenu #icons").css("display", "none");
				$("#castomMenu .talrashabgcastom").css("display", "none");
				$(`#castomMenu #icons .item.${this.current}`).removeClass(`active`);
				$(`#castomMenu #${this.current}`).css(`display`, `none`);
				$("#castomMenu .specifications-polzunok").slider("value", 50);
				$("#castomMenu .parents-polzunok").slider("value", 50);
				$("#castomMenu .rotation-polzunok").slider("value", 0);
				// this.current = undefined;
			},
		}
	});
	
	$("#castomMenu .specifications-polzunok").slider({
        min: 0,
        max: 100,
        value: 50,
        range: "min",
        ...
        slide: function(event, ui) {
			...
			castomApp.update("specifications", parseInt(id), ui.value)
        }
    });
	
	$("#castomMenu .parents-polzunok").slider({
        min: 0,
        max: 100,
        value: 50,
        range: "min",
        ...
        slide: function(event, ui) {
			var id = $(this).attr('data-id')
			castomApp.update(id, ui.value)
        }
    });
	
	$("#castomMenu .rotation-polzunok").slider({
        min: 0,
        max: 360,
        value: 0,
        range: "min",
        animate: "fast",
        slide: function(event, ui) {
			mp.trigger("castom:sliderValueChanged", ui.value);
        }
    });
});