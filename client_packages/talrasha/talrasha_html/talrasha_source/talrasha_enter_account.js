...
function showSpawnMenu() {
	$("#selectorCharacters").fadeOut(100).promise().done(function() {
		$("#setspawn").css("display", "flex");
		$("#setspawn").focus();
		setTimeout(function() {
			setCursor(true);
		}, 200);
		//mp.trigger(`fromBlur`, 100);
	});
}
...
function showAuthAccount(loginOrEmail = null, password = null) {
	...
	mp.trigger(`fromBlur`, 100); // Убирать если не стоит пароль
}
...
function authAccountHandler() {
    ...
    mp.trigger(`account.setAutoLogin`, true, loginOrEmail, password);
...
$(document).ready(function () {
	selectorCharacters = new Vue({
		...
		methods: {
			showSelectorCharacters: function(info) {
				this.data = JSON.parse(info);
                for (var i = 0; i < 3; i++) {
                    if (this.data.characters[i]) {
                        this.data.characters[i].level = convertMinutesToLevelRest(this.data.characters[i].minutes).level;
                        this.data.characters[i].regDate = convertMillsToDate(this.data.characters[i].regDate * 1000)
                    }
                }
                this.access = [true, this.data.isAchievements, this.data.isDonate];
