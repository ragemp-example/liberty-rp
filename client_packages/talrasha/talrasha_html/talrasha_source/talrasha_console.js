    ...
    window.consoleAPI = {
        ...
        pushReportMessage: (sqlId, messages) => {
            ...
            for (var i = 0; i < messages.length; i++) {
				...
				var playerId = messages[i].playerId;
				var serverId = messages[i].serverId;
				
                var el = $(`<div class="dialog-message">
                    <div class="text">
                        ${messages[i].text}
                    </div>
                    <div class="date">${name} ${playerId} (${serverId}) | ${convertMillsToDate(messages[i].date)}</div>
                </div>`);
        ...
        show: (enable) => {
            if (window.bindlocker(false, true)) return
            if (enable) {
        ...
        enable: (enable) => {
            ...
            if (enable) {
				...
				consoleAPI.consoleDisabled = false
            } else {
                consoleAPI.show(false);
                $(document).unbind('keydown', consoleAPI.showHandler);
				consoleAPI.consoleDisabled = true
            }
        },
		keyPressed: () => {
			if (consoleAPI.consoleDisabled) return;
