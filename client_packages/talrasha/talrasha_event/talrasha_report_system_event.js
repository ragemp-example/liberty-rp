exports = (menu) => {
	...
	mp.events.add("reportSystem.removeReport", (reportId) => {
         menu.execute(`playerMenu.removeReport(${reportId})`);
    });
    ...
    mp.events.add("reportSystem.reports", (reports) => {
        ...
        if(reports === undefined) {
            ...
            menu.execute(`window.events.call('playerMenu', {reports: ${JSON.stringify(reports)}, event: 'reports' })`);
        }
    });
    ...
    mp.events.add("reportSystem.messages", (messages) => {
        ...
        if(messages === undefined) {
            ...
            menu.execute(`window.events.call('playerMenu', {messages: ${JSON.stringify(messages)}, event: 'messages' })`);
        }
    });
    ...
    mp.events.add("reportSystem.close", (data) => {
        ...
        if(data === undefined) {
            ...
            menu.execute(`window.events.call('playerMenu', {closeTicket: ${JSON.stringify(data)}, event: 'closeTicket' })`);
        }
    });

    mp.events.add("reportSystem.addReport", (data) => {
        menu.execute(`window.events.call('playerMenu', {addReport: ${JSON.stringify(data)}, event: 'addReport' })`);
    });
}
