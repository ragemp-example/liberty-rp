...
const cuffControlsToDisable = [
    0, 21, 22, 24, 25, 26, 36, 37, 38, 44, 45,
    49, 75, 140, 263, 157, 158, 159, 160, 161,
    162, 163, 164, 165, 114, 121, 257, 14, 15,
    16, 17, 53, 54, 141, 142, 143, 145, 185,
    251, 47, 69, 70, 68, 92, 99, 115, 46, 331
];
exports = (menu) => {
    
    mp.events.add("render", () => {
        var player = mp.players.local;
        if (player.isFreeze || player.isBlockControl) mp.game.controls.disableAllControlActions(0);
    
        if (mp.clientStorage.hasCuffs) {
            for (var control of cuffControlsToDisable) {
	...
	mp.events.add("setBagHead", (set) => {
       menu.execute(`showBagHead(${set})`);
    })
    ...
    mp.events.add("render", () => {
        var player = mp.players.local;
        if (player.isFreeze || player.isBlockControl || mp.clientStorage.hasCuffs) {
        ...
        if (player.getVariable("attachedObject") && player.getVariable("attachedObject") !== 'prop_money_bag_01' && !throwAttachedObject && (/*player.isJumping() || */player.isShooting() || player.isSwimming()/* ||
                player.isFalling()*/)) {
...
addPlayerChatBubble = function(playerId, text) {
