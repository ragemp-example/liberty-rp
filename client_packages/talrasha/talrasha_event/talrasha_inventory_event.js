exports = (menu) => {
	...
	mp.events.add("inventory.changeState", (sqlId) => {
        menu.execute(`inventoryAPI.changeState('${sqlId}')`);
    });
	...
	mp.events.add("getCommonWeight", (value) => {
        menu.execute(`$("#getCommonWeight").text(${value}.toFixed(1))`);
    });
    ...
    mp.events.add("inventory.addVehicleItems", (items, veh, rows = 5, cols = 5) => {
